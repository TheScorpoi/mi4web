...
$url = "http://localhost:9874/?page=SioWiki&url=".urlencode("http://localhost:5000/send_hello");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="wrapper">
        ...
        <?php
            ...
            ?>
                    ...
                    <li class="nav-item" id="login">
                            <a class="nav-link" data-widget="control-sidebar" data-slide="true" onclick="document.location.href='<?php echo $url; ?>'" role="button">
                                Autenticar UAP
                            </a>
                        </li>
                        <li class="nav-item" id="login">
    ...
    $_SESSION['last_page'] = "publication.php?publication=".$publication;
    $_SESSION['last_id'] = $publication;
    $sql = "SELECT * FROM Publications WHERE id = $publication";
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <button class="btn btn-primary float-right" onclick="document.location.href = 'main.php'"><i class="fas fa-arrow-left"></i> Back</button>
                    </div>
                </div>
            </div>
            <!-- /.container-fluid -->
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="row mb-3 card-body">
                                        <div class="col-10">
                                            <?php
                                                echo "<p style='font-size: 24px;text-align: justify; text-justify: inter-word;'>". $row['content'] ."</p>";
                                            ?>
                                        ...
                                        <div class="col-2">
                                            ...
                                            <ul>
                                                <?php
                                                    $sql = "SELECT * FROM Files WHERE publication_id = $publication";
                                                    $result = mysqli_query($con, $sql);
                                                    while ($row = mysqli_fetch_assoc($result)) {
                                                        echo "<li><a href='download.php?filename=".$row['nome']."' target='_blank'>". $row['nome'] ."</a></li>";
                                                    }
                                                ?>
                                            </ul>
                                        </div>
                                    ...
                                    <div>
                                        <div class="timeline">
                                                    <?php
                                                        $sql = "SELECT * FROM Comments INNER JOIN Users ON Users.id = Comments.created_by WHERE Comments.publication_id = ".$publication;
                                                        $result = mysqli_query($con, $sql);
                                                        while($row = mysqli_fetch_assoc($result)) {
                                                            
                                                            echo "<div><i class='fas fa-comment bg-blue'></i>
                                                                    <div class='timeline-item'>
                                                                        <span class='time'>".$row['created_on']."</span>
                                                                            <h3 class='timeline-header'><b>".$row['username']."</b> commented</h3>
                                            ...
                                            <div>
                                                ...
                                                <div class="timeline-item">
                                                    <form method="post">
                                                    <div class="timeline-body">
                                                        <textarea name="comment" id="comment" cols="170" onkeyup="required1()" rows="5" placeholder="Escreva aqui o seu comentário..."></textarea>
                                                        <br>
                                                        <div class="ml-auto" style="right: 0">
                                                            <input type="submit" class="btn btn-primary btn-sm ml-auto" disabled="disabled" id="btn_submit" value="enviar" />
                                                        </div>
                                                        <?php
                                                        if (!empty($_POST['comment'])) {
                                                            $comment = htmlspecialchars($_POST['comment']);
                                                            //$q = sprintf("SELECT * FROM Users WHERE username='%s' AND pass='%s'", mysqli_real_escape_string($con, $username),mysqli_real_escape_string($con, $pass));
                                                            $sql = sprintf("INSERT INTO Comments (content, created_on, created_by, publication_id) VALUES ('%s', NOW(), '%s', '%s')", mysqli_real_escape_string($con, $comment),mysqli_real_escape_string($con, $_SESSION['id']),mysqli_real_escape_string($con, $publication));
                                                            $result = mysqli_query($con, $sql);
                                                            if ($result) {
                                                            ...
                                                            $_POST['comment'] = "";
                                                            header("Location: publication.php?publication=".$publication);
                                                        }
                                                        ?>
                                                    </div>
                                                    </form>
