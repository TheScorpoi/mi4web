...
if (isset($params['token'])) {
    ...
    $token = $params['token'];
    $user='bernas';
    $q = sprintf("SELECT * FROM Users WHERE token='%s'", mysqli_real_escape_string($con, $token));
...
if (isset($_SESSION['last_page']) && $_SESSION['last_page'] != "main.php" && isset($params['token'])) {
    $tmp = $_SESSION['last_page'];
    $_SESSION['last_page'] = "main.php";
    header("Location: http://localhost:8000/".$tmp);
}
else {
    $_SESSION['last_page'] = "main.php";
}
            ...
            ?>
                    <ul class="navbar-nav ml-auto">
                        <!--<li class="nav-item" id="login">
                            <a class="nav-link" data-widget="control-sidebar" data-slide="true" onclick="document.location.href='login.php'" role="button">
                                Login
                            </a>
                        </li>-->
                        <li class="nav-item" id="login">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        ...
                        <div class="col-sm-6">
                            <button class="btn btn-primary float-right" onclick="document.location.href = 'create_publication.php'"><i class="fas fa-plus"></i> Adicionar</button>
                        </div>
                    </div>
                </div>
                <!-- /.container-fluid -->
                                            ...
                                            <?php
                                                ...
                                                if ($result->num_rows > 0) {
                                                    while($row = $result->fetch_assoc()) {
                                                        echo "<tr onclick='see_publication(".$row["id"].")'>
                                                                <td>
