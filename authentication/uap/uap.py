...
server_hello ='http://localhost:5000/send_hello'
server_challenge = ''
server_done = ''
...
def show_login():
    ...
    if logged_in and os.path.isfile('bd.json') and os.stat('bd.json').st_size != 0:
        ...
        key_bd = base64.urlsafe_b64encode(kdf.derive(password_key.encode("utf-8")))
...
def update_password():
    ...
    if request.method == 'POST':
        ...
        # guardar a nova pass
        to_store = JSON.dumps({'password': new_password}, indent=4, sort_keys=True).encode('utf-8')
        ...
        if os.path.isfile('bd.json') and os.stat('bd.json').st_size != 0:
            ...
            key_bd = base64.urlsafe_b64encode(kdf.derive(new_password.encode("utf-8")))
...
def store_password():
    ...
    _salt = os.urandom(16) # o salt só é calculado uma vez e depois tem de ser sempre o mesmo

    to_store = JSON.dumps({'password': password}, indent=4, sort_keys=True).encode('utf-8')
...
def get_informations():
    ...
    if request.form.get('store_credentials'):
        ...
        if logged_in:
            ...
            bd_file = open("bd.json", "wb")
            content = JSON.dumps(bd, indent=4, sort_keys=True).encode('utf-8') # para ler temos de fazer decode
    ...
    if retorno:
        result, server_token_tmp = calculate_challenge(user, challenge, token_tmp)
        if result and valores_errados == False:
            ...
            serverFicticio = True
            valores_errados = False
            return render_template('login.html', page = page1, serverFicticio=serverFicticio, bd=bd[page1], logged_in=logged_in) # fazer warning a dizer que o user não existe no server
        else:
            #!este é para quando a password esta mal
            serverFicticio = False
            incorrectPassword = True
            valores_errados = False
            return render_template('login.html', page = page1, incorrectPassword=incorrectPassword, bd=bd[page1], logged_in=logged_in) # fazer warning a dizer que o user não existe no server  
    else:
        userNotFound = True
        return render_template('login.html', page = page1, userNotFound=userNotFound, bd=bd[page1], logged_in=logged_in) # fazer warning a dizer que o user não existe no server
...
def send_hello(user):
    ...
    uap_url = request.url_root[0:-1] + url_for('calculate_challenge') #  [0:-1] para retirar a barra (/)
...
def calculate_challenge(user, challenge, token_tmp):
    ...
    while True:    
        ...
        if int(bit) == int(bit_to_send) and valores_errados == False:
            ...
            bits = BitArray(hex=digest.hexdigest())
            bit_to_send = bits.bin[0]
            dict = {"challenge": next_challenge, "bit": bit_to_send, 'token_tmp': token_tmp }
        else:
