...
app.get("/testar_insert", (req, res) => {
    let post = { email: "santos@incesto.pt", fullname: "Fernando Santos", password: "12345" };
...
app.get("/testar_insert_with_params/:email/:fullname/:password", (req, res) => {
    let post = { email: req.params.email, fullname: req.params.fullname, password: req.params.password };
    let sql_query = 'INSERT INTO `user` SET ?';
    db.query(sql_query, post, (error, results) => {
        if (error) throw error;
        res.send(results);
